"""Virtual staining rebuttal — leave-one-out Pearson correlation evaluation.
...
Usage::
    ...
    python rebbutal.py configs/train_vit_config.yaml \\
...
def main():
    ...
    for batch_idx, (img, channel_ids, dataset_name, img_path) in enumerate(tqdm(test_dataloader)):
        # Denormalize for Pearson: reverse clip normalization
        ...
        batch_size, num_channels_img, H, W = img.shape
        ...
        for c in range(num_channels_img):
            ...
            mi_np = mi.float().cpu().numpy() * clip_limit       # denormalize
