    ...
    for i in range(0, len(ax_flat), 3):
        ...
        if j < num_channels:
            ...
            if channel_id in fully_masked_ids:
                ...
                rgba_img = cmap(img_data)
                mask_np = pixel_masks[0, masked_idx].cpu().numpy()
                rgba_img[mask_np] = [1.0, 1.0, 1.0, 1.0]
...
def init_experiment(config: dict[str, Optional[str]]) -> None:
...
def log_training_metrics(
    ...
    aleatoric_unc: float | None = None,
    epistemic_unc: float | None = None,
    total_unc: float | None = None,
    evidence_nu: float | None = None,
    ...
    **kwargs,
    ...
    Args:
        ...
        logvar (float | None): Log variance (beta-NLL mode)
        aleatoric_unc (float | None): Aleatoric uncertainty (evidential mode)
        epistemic_unc (float | None): Epistemic uncertainty (evidential mode)
        total_unc (float | None): Total uncertainty (evidential mode)
        evidence_nu (float | None): Evidence parameter nu (evidential mode)
        alpha (float | None): Shape parameter alpha (evidential mode)
    """
    ...
    if logvar is not None:
        metrics["train/logvar"] = logvar
    if aleatoric_unc is not None:
        metrics["train/aleatoric_uncertainty"] = aleatoric_unc
    if epistemic_unc is not None:
        metrics["train/epistemic_uncertainty"] = epistemic_unc
    if total_unc is not None:
        metrics["train/total_uncertainty"] = total_unc
    if evidence_nu is not None:
        metrics["train/evidence_nu"] = evidence_nu
...
def log_validation_metrics(
    ...
    aleatoric_mae_correlation: float | None = None,
    epistemic_mae_correlation: float | None = None,
    mean_aleatoric: float | None = None,
    ...
    **kwargs,
    ...
    Args:
        ...
        variance_mae_correlation (float | None): Pearson correlation (beta-NLL mode)
        aleatoric_mae_correlation (float | None): Aleatoric-MAE correlation (evidential mode)
        epistemic_mae_correlation (float | None): Epistemic-MAE correlation (evidential mode)
        mean_aleatoric (float | None): Mean aleatoric uncertainty (evidential mode)
        mean_epistemic (float | None): Mean epistemic uncertainty (evidential mode)
    """
    if _experiment is None:
        return
    ...
    if aleatoric_mae_correlation is not None:
        metrics["val/aleatoric_mae_correlation"] = aleatoric_mae_correlation
    if epistemic_mae_correlation is not None:
        metrics["val/epistemic_mae_correlation"] = epistemic_mae_correlation
    if mean_aleatoric is not None:
        metrics["val/mean_aleatoric"] = mean_aleatoric
    if mean_epistemic is not None:
        metrics["val/mean_epistemic"] = mean_epistemic
