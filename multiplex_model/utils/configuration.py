...
class TrainingConfig(BaseModel):
    ...
    beta: float = Field(default=1.0, ge=0, description="Beta parameter for beta-NLL loss")

    # Uncertainty method
    uncertainty_method: str = Field(
        default="evidential",
        description="Uncertainty method: 'evidential' (NIG, 4 outputs) or 'beta_nll' (Gaussian, 2 outputs)",
    )
    evidence_reg_coeff: float = Field(
        default=0.01,
        ge=0,
        description="Evidence regularization coefficient for evidential loss",
    )
    ...
    mask_strategy: str = Field(
        default="learnable",
        description=(
            "Spatial masking strategy: "
            "'zero' (fill masked pixels with 0.0), "
            "'negative' (fill with -1.0), "
            "'learnable' (fill with 0.0 + replace features with learnable mask token in encoder)"
        ),
    )
    ...
    output_activation: str = Field(
        default="swishoid",
        description=(
            "Activation for predicted mean: "
            "'sigmoid' (classic, never reaches 0), "
            "'hard_sigmoid' (piecewise-linear, reaches 0 and 1), "
            "'sigmoswish' (x·σ(βx) clamped to [0,1]), "
            "'swishoid' (smooth, no clamp, recommended for zero-inflated data)"
        ),
    )
    activation_beta: float = Field(
        default=4.0,
        ge=0.01,
        description="Temperature for sigmoswish/swishoid activation. Higher = faster saturation.",
    )
    activation_window: float = Field(
        default=0.5,
        ge=0.01,
        description="Half-width for swishoid zero-crossing control. Ignored by other activations.",
    )
    learnable_activation_params: bool = Field(
        default=False,
        description=(
            "When True, activation parameters (beta and, for swishoid, window) "
            "become learnable nn.Parameters optimised jointly with the model. "
            "Allows the network to discover the optimal activation shape."
        ),
    )
    ...
    def validate_mask_strategy(cls, v: str) -> str:
        allowed = ("zero", "negative", "learnable")
        if v not in allowed:
            raise ValueError(f"mask_strategy must be one of {allowed}, got '{v}'")
        return v

    @field_validator("output_activation")
    @classmethod
    def validate_output_activation(cls, v: str) -> str:
        allowed = ("sigmoid", "hard_sigmoid", "sigmoswish", "swishoid")
        if v not in allowed:
            raise ValueError(f"output_activation must be one of {allowed}, got '{v}'")
        return v
    ...
    model_config = ConfigDict(extra="forbid", populate_by_name=True)  # Raise error on unknown fields
