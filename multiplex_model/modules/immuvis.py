...
class MultiplexImageEncoder(nn.Module):
    ...
    def __init__(
        ...
        # Learnable spatial mask token — replaces features at masked positions
        # after the marker-agnostic encoder, so the Hyperkernel and pan-marker
        # encoder receive a distinctive "masked" signal instead of features
        # derived from zero-filled pixels (which are ambiguous with genuine
        # zero-background in mass spectrometry data).
        self.spatial_mask_token = nn.Parameter(
            torch.zeros(1, 1, hyperkernel_input_dim, 1, 1)
        )
    ...
    def forward(
        ...
        Args:
            ...
            spatial_mask (torch.Tensor | None, optional): Boolean mask of spatially masked pixels
                with shape [B, C, H, W]. True = masked. When provided, masked positions in the
                feature map are replaced with a learnable mask token before the Hyperkernel,
                resolving the ambiguity between genuine background zeros and masking zeros.
        ...
        x = x.reshape(B, C, E_ma, H_ma, W_ma)

        # Inject learnable mask token at masked spatial positions
        if spatial_mask is not None:
            # Downsample pixel-level mask to match post-MaE feature resolution
            # spatial_mask: (B, C, H, W) bool → (B, C, H_ma, W_ma) bool
            if H_ma != H or W_ma != W:
                mask_ds = F.adaptive_max_pool2d(
                    spatial_mask.float(), (H_ma, W_ma)
                ) > 0.5
            else:
                mask_ds = spatial_mask

            # Replace masked positions with the learnable mask token
            # mask_ds: (B, C, H_ma, W_ma) → expand to (B, C, E_ma, H_ma, W_ma)
            mask_expanded = mask_ds.unsqueeze(2).expand_as(x)
            x = torch.where(
                mask_expanded,
                self.spatial_mask_token.expand_as(x),
                x,
            )
...
class MultiplexAutoencoder(nn.Module):
    ...
    def __init__(
        ...
        # Rebrand
        explicit_scaling = decoder_config.pop("scaling_factor", None)
        final_scaling = explicit_scaling if explicit_scaling is not None else scaling_factor
    ...
    def encode(
        ...
        Args:
            ...
            spatial_mask (torch.Tensor | None, optional): Boolean mask [B, C, H, W] indicating
                spatially masked pixels. True = masked. Enables learnable mask token injection.
    ...
    def forward(
        ...
        Args:
            ...
            decoded_indices (torch.Tensor): Indices of the markers in channels
                for decoding.
            spatial_mask (torch.Tensor | None, optional): Boolean mask [B, C, H, W]
                indicating spatially masked pixels. Enables learnable mask token.
