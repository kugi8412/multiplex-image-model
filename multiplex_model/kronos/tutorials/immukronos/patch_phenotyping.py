"""
Patch Phenotyping pipeline for ImmuKronos (DINOv3).

Mirrors the original KRONOS patch phenotyping tutorial:
    1. Extract patches from tissue images
    2. Extract marker-level features using ImmuKronos backbone
    3. Train logistic regression classifier with Optuna
...
class PatchPhenotypingDataset(PatchDataset):
    """Alias for PatchDataset used in patch phenotyping pipeline."""
    pass


class PatchPhenotypingFeatures(Dataset):
    """Loads pre-extracted features with associated labels."""
...
class PatchPhenotypingClassifier:
    """
    Logistic regression classifier for patch phenotyping.

    Config keys:
        feature_dir: Directory with .npy features.
        output_dir: Directory for results.
        n_trials: Optuna trials (default: 50).
        max_iter: LogisticRegression max_iter (default: 5000).
    """
    ...
    def _load_features(self, df):
        ds = PatchPhenotypingFeatures(
            self.config["feature_dir"],
            df["patch_name"].tolist(),
            df["label"].tolist(),
        )
        return ds.get_all()
...
class PatchPhenotyping:
    """
    High-level patch phenotyping pipeline.

    Config keys:
        checkpoint_path: Path to ImmuKronos checkpoint.
        patch_dir: Directory with .h5 patches.
    ...
    def extract_features(self):
        ...
        with torch.no_grad():
            for patches, marker_ids, patch_names in tqdm(
                dataloader, desc="Patch feature extraction"
            ):
        ...
        print(f"Patch features saved to {feature_dir}")
    ...
    def train_and_evaluate(self, train_df, valid_df, test_df, fold_name="fold_0"):
        ...
        classifier = PatchPhenotypingClassifier(
